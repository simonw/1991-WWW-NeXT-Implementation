...
**				===========================
**
**	As much as possible, this is written in C for portability.
**
*/

#import "HTStyle.h"

#define INFINITY 1024		/* Aaaaaaghrh! @@@@@@ */
...
static SGML_style * styleTable[] = {
	...
	&Glossary, &listStyle, &addressStyle, &Example, &Listing
}; /* style table */
...
void update_style()
{
    ...
    if (current_style->free_format && cur && next) {
    	int i;
...
//    if (current_style->free_format) {				O L  D
//      int newlines = (next?next->spaceBefore/next->paragraph->lineHt:0);
//      for(i=0; i<newlines; i++) OUTPUT('\n');	/* Rather approximate! 	*/
//	output_in_word=0;
//    }
...
*/
    char title[INFINITY];			/* See <TITLE> tag. */
    char reference[INFINITY];		/* See <A HREF=...> attribute */
    char anchor_name[INFINITY];		/* See <A NAME=...> attribute */
    int title_length = 0;
...
*/
	...
	case S_word:		/* We have just had non-white characters */
	    if (!WHITE(c) && (c!= '<')) {
	        OUTPUT(c);
		break;
	    }			/* Else FALL THROUGH to text handling !!!! */
	...
	case S_tag_n:
	    ...
	    case 'e':
			...
			if ((c<'0') || (c>'9')) {
			    ...
			    if (TRACE) printf("Next anchor number: %i\n", value);
			    BACK_UP;
			    SETSTATE(S_junk_tag);
			    break;
			}
...
*/
	case S_anchor:
		...
		if (sscanf(anchor_name, "%i", &anchorNumber) > 0)	/* numeric? */
	...
	case S_href:
	    ...
	    if (reference_length<INFINITY-1) {
	...
	case S_aname:
	    ...
	    if (anchor_name_length<INFINITY-1) {
...
*/
	case S_title:
		...
		if (title_length < INFINITY-1) title[title_length++] = c; 
		SETSTATE( state);
	    } /* if */
	case S_done:
...
void change_run(NXRun *last, NXRun *r)
{
	...
	if (r->info) {
	    ...
	    NXPrintf(sgmlStream, "<A NAME=%s", [a address]);
	    if (d) {
	        Anchor * p = [d parent];
